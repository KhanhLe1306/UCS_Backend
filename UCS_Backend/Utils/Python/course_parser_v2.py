...
def process():
    ...
    for course, lectures in courses.items():
        for lecture in lectures:
            ...
            if ROOM in rooms:
                rooms[ROOM].append((course, TIME, WEEKDAY))
            else:
                if ROOM != 'NULL':
                    rooms[ROOM] = [(course, TIME, WEEKDAY)]
    for room in rooms:
        rooms[room] = sorted(rooms[room], key=lambda item: item[1][0])
            
            
def print_data(schedule, time_blockings, availability, roominfo):
    global courses_revised
    global weekday_time_blockings
    global rooms
    if schedule:
